...
############################################
## Stochastic volatility : multi-factor model
# 
...
def subtransitionAndWeight(states, y, parameters, alluniforms1, allK1, alluniforms2, allK2):
    ...
    for(int indexk = 0; indexk < Nx; indexk ++){
        ...
        tempmeasure1 = y(0) - (parameters(0) + parameters(1) * states(indexk, 0) + parameters(7) * sum11 + parameters(8) * sum12
        - parameters(2) * (parameters(6) * parameters(7) * parameters(4) + (1 - parameters(6)) * parameters(8) * (parameters(4) + parameters(5))));
